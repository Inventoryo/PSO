...
%     rectangle('Position',[42000-4000,30000-4000,8000,8000],'Curvature',[1,1],'FaceColor',[0.41176,0.41176,0.41176]),axis equal;
%     rectangle('Position',[14000,14000,4000,4000],'FaceColor',[0,0,0]);

%       rectangle('Position',[5000-3000,30000-3000,6000,6000],'Curvature',[1,1],'FaceColor',[0.41176,0.41176,0.41176]),axis equal;
...
%     axis equal;
% %     
% %    % legend([p010,p011,p012,p013,p014,p015,p016,p017,p018,p019,p020,p021],'TARGET1', 'TARGET2','TARGET3', 'TARGET4','TARGET5', 'TARGET6','TARGET7','TARGET8','TARGET9','TARGET10','TARGET11','TARGET12');
% %    % hl = legend([p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15],'U1', 'U2','U3', 'U4','U5', 'U6','U7','U8', 'U9','T1','T2','T3','T4','T5','T6','Location','eastoutside');
...
% %     
%    % hl = legend([p1,p2,p3,p4,p5,p6,p7,p8,p9,p91,p10,p11,p12,p13,p14,p15],'UAV1', 'UAV2','UAV3', 'UAV4','UAV5', 'UAV6','UAV7','UAV8', 'UAV9','UAV10','TARGET1','TARGET2','TARGET3','TARGET4','TARGET5','TARGET6','Location','northoutside');
%     
...
%     hl = legend([p1,p2,p3,p4,p5,p6],'U1', 'U2','U3', 'U4','U5', 'U6','Location','northoutside');
%     set(hl,'Orientation','horizon');
...
%         text(5000,30000,'rader','color','g');
      rectangle('Position',[5000-3000,30000-3000,6000,6000],'Curvature',[1,1],'FaceColor',[0.41176,0.41176,0.41176]),axis equal;
      rectangle('Position',[30000-4000,15000-4000,8000,8000],'Curvature',[1,1],'FaceColor',[0.41176,0.41176,0.41176]),axis equal;
      rectangle('Position',[40000-4000,30000-4000,8000,8000],'Curvature',[1,1],'FaceColor',[0.41176,0.41176,0.41176]),axis equal;
      rectangle('Position',[5000-3000,15000-3000,6000,6000],'Curvature',[1,1],'FaceColor',[0.41176,0.41176,0.41176]),axis equal;
      rectangle('Position',[30000,45000,3000,3000],'FaceColor',[0,0,0]);
       % rectangle('Position',[30000,45000,2000,2000]);
        for i=1:uav_num
            end_x = (i-1)*3+1;
            end_y = (i-1)*3+2;
            ...
            %text(uav(j,(i-1)*3+1),uav(j,(i-1)*3+2),['(target',num2str(uav(j,(i-1)*3+3)),')'],'color',color(i));
            plot(Gbest_position(j,(i-1)*2+1),Gbest_position(j,(i-1)*2+2),[color(i),'o']);
            %plot(area_Point(j,(i-1)*2+1),area_Point(j,(i-1)*2+2),[color(i),'*']);
        end
        
        target_num = length(target(1,:))/2;
        for i=1:target_num
            end_x = (i-1)*2+1;
            end_y = (i-1)*2+2;
            
            plot(target(1:j,end_x),target(1:j,end_y),[color(i),'p']);
            text(target(j,end_x),target(j,end_y)+1000,['target',num2str(i),''],'color',color(i));
        end
