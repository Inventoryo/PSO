...
utility::point2D PIO::findBestPoints(const vector<cv::Mat> &layers, const utility::State & uav_state, int border, int resolution) {
...
double PIO::getUncertanty(const vector<cv::Mat> &layers, const double x, const double y, int border, int resulution, int used_layer_num){
	double res = 0 ;
...
void PIO::initParams(const string& config_file_path){
...
void PIO::create(const string& config_file_path)
{
...
void PIO::getNextPoint(utility::MAP &global_map, utility::RADAR *radar, utility::OBSTACLE *obs, vector<utility::TARGET> &target, vector<utility::UAV> &uav, int cur_T, int uav_idx)
{
...
double PIO::calculateFitness(particle* particle){
...
void PIO::updateParticleStatesAtMapMode(particle* particle, int iteration)
{
	...
	line_X = line_X * exp(R_ * iteration) + r1 * (particle->Pbest_state.position.x - particle->state.position.x);
	line_Y = line_Y * exp(R_ * iteration) + r1 * (particle->Pbest_state.position.y - particle->state.position.y);
...
void PIO::updateParticleStatesAtLandmarkMode(particle* particle, int iteration, utility::point2D& center){
	double r2 = rand() % 1000 / 1000.0;
	particle->state.position = particle->state.position + (center - particle->state.position) * r2;
